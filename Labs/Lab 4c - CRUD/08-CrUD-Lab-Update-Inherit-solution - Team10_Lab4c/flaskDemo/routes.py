...
from flaskDemo.forms import AssignForm, RegistrationForm, LoginForm, UpdateAccountForm, PostForm, DeptForm, DeptUpdateForm
...
def home():
    ...
    results = Works_On.query.join(Project, Works_On.pno == Project.pnumber) \
        .add_columns(Works_On.pno, Works_On.essn, Project.pnumber, Project.pname) \
        .join(Employee, Employee.ssn == Works_On.essn) \
        .add_columns(Employee.fname, Employee.lname, Employee.ssn)
    return render_template('home.html', joined_m_n=results)
...
def new_dept():
    form = DeptForm()
    if form.validate_on_submit():
        dept = Department(dname=form.dname.data, dnumber=form.dnumber.data,
                          mgr_ssn=form.mgr_ssn.data, mgr_start=form.mgr_start.data)
        db.session.add(dept)
        db.session.commit()
...
def dept(dnumber):
    ...
    results = Works_On.query.join(Project, Works_On.pno == Project.pnumber) \
        .add_columns(Works_On.pno, Works_On.essn, Project.pnumber, Project.pname) \
        .join(Employee, Employee.ssn == Works_On.essn) \
        .add_columns(Employee.fname, Employee.lname, Employee.ssn, Employee.dno) \

    return render_template('dept.html', title=dept.dname, dept=dept, results=results, now=datetime.utcnow())
...
def new_assign():
    ...
    if form.validate_on_submit():
        assignment = Works_On(essn=form.essn.data, pno=form.pno.data,
                              hours=form.hours.data)
        db.session.add(assignment)
        db.session.commit()
        flash('You have assigned employee ' + form.essn.data +
              ' to project ' + form.pno.data + '!', 'success')
        return redirect(url_for('home'))
