...
from flaskDemo.models import Employee, Project, User, Department, getDepartment, getDepartmentFactory
...
class DeptUpdateForm(FlaskForm):
    ...
    dname = StringField('Department Name:', validators=[
...
#  One of many ways to use SelectField or QuerySelectField.  Lots of issues using those fields!!
    # myChoices defined at top
    mgr_ssn = SelectField("Manager's SSN", choices=myChoices)
...
# got rid of def validate_dnumber
    ...
    # apparently in the company DB, dname is specified as unique
    ...
    def validate_dname(self, dname):
        ...
        if dept and (str(dept.dnumber) != str(self.dnumber.data)):
            raise ValidationError(
                'That department name is already being used. Please choose a different name.')
...
class DeptForm(DeptUpdateForm):
    ...
    # because dnumber is primary key and should be unique
    def validate_dnumber(self, dnumber):
        dept = Department.query.filter_by(dnumber=dnumber.data).first()
        if dept:
            raise ValidationError(
                'That department number is taken. Please choose a different one.')


employee_ssns = Employee.query.with_entities(Employee.ssn).distinct()
employee_ssns_results = list()
for row in employee_ssns:
    rowDict = row._asdict()
    employee_ssns_results.append(rowDict)
employee_ssns_choices = [(row['ssn'], row['ssn'])
                         for row in employee_ssns_results]

project_numbers = Project.query.with_entities(Project.pnumber).distinct()
project_numbers_results = list()
for row in project_numbers:
    rowDict = row._asdict()
    project_numbers_results.append(rowDict)
project_numbers_choices = [(row['pnumber'], row['pnumber'])
                           for row in project_numbers_results]


class AssignForm(FlaskForm):
    essn = SelectField("Employee SSN", choices=employee_ssns_choices)
    pno = SelectField("Project Number", choices=project_numbers_choices)
    hours = DecimalField("Hours Worked", validators=[DataRequired()])

    submit = SubmitField('Assign Employee to Project')
